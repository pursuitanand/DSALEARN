...
public class myQueue {
    /**
     * Correct solution: Circular Queue , Remember % capacity makes them go in a circle.
     * Queue rule:
     * Rear enters. Front exits. % capacity makes them go in a circle.
     * template becomes:  enqueue → rear moves → put value → size++
     * dequeue → front moves → size--
     * empty → size == 0
     * full → size == capacity
     */
    ...
    public static void main(String[] args) {
        ...
        queue.enqueue(3);
        queue.enqueue(4);
        System.out.println(queue.getFront()); // Output: 5
        queue.dequeue();
        System.out.println(queue.isEmpty()); // Output: false
        System.out.println(queue.getRear()); // Output: 4

        // Test Edge Cases
        myQueue queue2 = new myQueue(2);
        System.out.println(queue2.getRear()); // Output: -1 (Queue is empty)
        queue2.enqueue(3);
        queue2.enqueue(7);
        System.out.println(queue2.isFull()); // Output: true (Queue is full)
    }
}
